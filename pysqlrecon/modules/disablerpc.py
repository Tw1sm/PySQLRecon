...
app = typer.Typer()
COMMAND_NAME = "disablerpc"
HELP = "[red][PRIV][/] Disable RPC and RPC Out on a linked server [I]"
...
def main(
    ctx: typer.Context,
    link: str = typer.Option(..., "--link", "-l", help="Linked server to disable RPC on")):
    ...
    logger.info(f"Disabling RPC on {link}")
    pysqlrecon.module_toggle("rpc", "false", link_host=link)
