...
app = typer.Typer()
COMMAND_NAME = "enableclr"
HELP = "[red][PRIV][/] Enable CLR integration [I,L]"
...
def main(ctx: typer.Context):
    ...
    if pysqlrecon.link is not None:
        ...
        logger.info(f"Enabling CLR integration on {pysqlrecon.link} via {pysqlrecon.target}")
        pysqlrecon.linked_module_toggle("clr enabled", "1")

        pysqlrecon.linked_check_module("clr enabled")
        pysqlrecon.print_results(use_basic_tables)

    else:
        logger.info(f"Enabling CLR integration on {pysqlrecon.target}")
        pysqlrecon.module_toggle("clr enabled", "1")
